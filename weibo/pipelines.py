...
class CsvPipeline(object):
    def process_item(self, item, spider):
        base_dir = '结果文件' + os.sep + item['keyword'] + settings.get('START_DATE')
        if not os.path.isdir(base_dir):
            os.makedirs(base_dir)
        file_path = base_dir + os.sep + item['keyword'] + settings.get('START_DATE') + '-' + settings.get('START_TIME') + '.csv'
...
class MysqlPipeline(object):
    ...
    def create_table(self):
        """创建MySQL表"""
        sql = """
                CREATE TABLE IF NOT EXISTS real_time_weibo (
    ...
    def process_item(self, item, spider):
        ...
        sql = """INSERT INTO {table}({keys}) VALUES ({values}) ON
                     DUPLICATE KEY UPDATE""".format(table='real_time_weibo',
                                                    keys=keys,
                                                    values=values)
