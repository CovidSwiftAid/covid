...
def ExcuteSingleQuery(locationList):
    ...
    currentUrl = base + "output=" + output + "&batch=" + batch + "&address=" + locationString + "&key=" + "220be4d56ace3959ff863b6994404f49"
    # 3-提交请求
    ...
    if answer['status'] == '1' and answer['info'] == 'OK':
        ...
        for i in range(0, len(tmpList)):
            try:
                ...
                level = tmpList[i]['level']
                # 放入结果序列
                resultList.append([coordList[0], coordList[1], level])
            except:
                # 如果发生错误则存入None
...
if __name__ == '__main__':
    ...
    # 进行地理编码
    print(ExcuteSingleQuery(locationList=["四川省成都市崇州市崇阳街道晋康北路313号"]))
